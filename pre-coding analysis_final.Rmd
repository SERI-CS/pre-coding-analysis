---
title: "incubation analysis_final"
author: "JZ"
date: "3/15/2022"
...
# overall
summary(lm(dateDiff~ as.factor(Unit) + NDaysPiazza+NDaysCanvas + NViews + NMinutes, data = df_startDate))

#Unit 0 
df_0 = subset(df_startDate, Unit == 2) %>% mutate(NMinutes != 0)
all.reg <- lm(dateDiff~ NDaysPiazza+NDaysCanvas + NViews + NMinutes, data = df_0)
...
for (i in c(1:9)) {
  ...
  r = summary(lm(dateDiff~ NDaysPiazza + NDaysCanvas + NViews + NMinutes, data = temp))
...
summary(lm(rank~as.factor(Unit) + NDaysPiazza + NDaysCanvas + NViews + NMinutes, data = df_gradeRanks))

#HW 0
df_grade_3 = subset(df_gradeRanks, Unit == 3)

all.reg <- lm(rank~ NDaysPiazza + NDaysCanvas + NViews + NMinutes, data = df_grade_3)
...
for (i in c(1:9)) {
  ...
  r = summary(lm(rank~ NDaysPiazza + NDaysCanvas + NViews + NMinutes, data = temp))
  print(i)
  print(r)
...
for (i in c(1:9)){
  ...
  for(x in c(7,8,9,12)){
    ...
    r = summary(lm(temp$rank ~ predictor_column))
